...
{
    // restoreOriginalSortOrder();
    // for (int i = 0; i < size(); ++i) {
    //     CardSetPtr set = at(i);
    //     set->setSortKey(i);
    // }
    // sort by set type priority followed by date.
    QDate epoch(1990, 1, 1);
    QDate yearFromNow = QDate::currentDate().addYears(1);
    qint64 fromEpoch = epoch.daysTo(yearFromNow);
    ...
    for (int i = 0; i < size(); ++i) {
        ...
        qint64 toYearFromNow = date.daysTo(yearFromNow);
        CardSet::Priority priority = set->getPriority();
        if (!date.isValid()) {
            priority = CardSet::Priority::LOWEST;
        }

        set->setSortKey(static_cast<unsigned int>(toYearFromNow + fromEpoch * priority));
    }
}

void SetList::restoreOriginalSortOrder()
{
